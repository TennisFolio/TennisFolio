# Feature: 모임 관리자 참가자 추가·수정

## 1. Goal

모임 관리자는 참가자가 직접 응답하지 않아도 클럽 멤버 또는 게스트를 명단에 추가하고, 등록된 참가자의 허용 정보를 수정할 수 있다.
...
- 클럽 모임에서는 활성 클럽 관리자만 등록할 수 있다. 클럽 멤버 요청에는 `clubMemberId`, `attendanceStatus`가 필요하며 서버가 멤버의 이름·성별을 사용한다. 게스트는 선택적으로 해당 클럽의 `clubSkillTierId`를 보낼 수 있고, 서버는 현재 클럽에 존재하는 등급만 허용한다.
...
- 관리 명단에서 모임장 본인을 제외한 참가자를 선택하면 수정 바텀시트를 연다. 게스트는 이름·성별·상태·등급을 수정하고, 클럽원은 상태만 수정한다. `PATCH /api/meetings/{publicId}/participants/{attendanceId}`는 이 관리자 수정 흐름만 담당한다.
...
- 게스트 참석자는 선택한 등급 ID만 보관한다. 명단 응답은 현재 클럽 등급을 조회해 이름을 붙이며, 삭제된 등급은 `등급 미정`으로 표시한다.
- 명단의 삭제 버튼은 수정 선택과 분리한다. 참가자 수정은 참가자 유형을 바꾸거나 클럽원 기본 정보를 변경하지 않는다.
...
- [ ] `feat: add club guest skill tier`
  - 구현: 게스트 등급 검증·저장·응답, 관리 바텀시트 선택, 명단 칩 표시
  - 테스트: 클럽 게스트 저장, 개인 모임·다른 클럽 또는 없는 등급 거절
  - 검증: 관련 `MeetingAttendanceCommandService` 테스트

- [ ] `feat: edit managed meeting participants`
  - 구현: 관리자 수정 API, 게스트·클럽원별 수정 바텀시트, 명단 선택 안내
  - 테스트: 게스트 전체 수정, 클럽원 상태 수정, 권한·불변 필드·정원·중복 실패
  - 검증: 관련 `MeetingAttendanceCommandService` 테스트
...
- 2026-08-01: `MeetingAttendanceCommandServiceTest`, `MeetingControllerTest` 집중 회귀 테스트 통과.
- 2026-08-01: 클럽 관리자 삭제 권한, 키보드 삭제 전파, 삭제된 게스트 등급 저장값을 보완하고 관련 백엔드 회귀 테스트를 다시 통과했다.
- 프런트엔드 build/test와 모바일 수동 확인은 실행하지 않았다.
...
| 2026-07-31 | 클럽 모임 게스트 등급 추가 | 게스트의 실력 정보를 모임 명단에도 남김 |
| 2026-07-31 | 관리자 참가자 수정 설계 추가 | 명단에서 게스트와 클럽원의 허용 정보를 수정할 수 있게 함 |
